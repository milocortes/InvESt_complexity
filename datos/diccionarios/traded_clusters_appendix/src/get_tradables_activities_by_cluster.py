...
for cluster in data_sheetnames:
    ...
    traded_cluster_all[cluster] = cluster_data

del traded_cluster_all['Househld Textiles & Leather Pro']
...
def get_transables_by_cluster(cluster_name):
    print(cluster_name)
    # Obtenemos todas las clases que son tradables para el cluster
    traded_cluster = {cluster_name : traded_cluster_all[cluster_name]}
    clases_transables = list(traded_cluster[cluster_name].naics.unique())
    ...
    if 541711 in clases_transables:
        clases_transables.remove(541711)

    return clases_transables

## Guardamos las clases transables de cada cluster en un diccionario para guardarlo como json

clusters_transables_scian = {i : get_transables_by_cluster(i) for i in traded_cluster_all.keys()}

import json 

json.dump(clusters_transables_scian, open(os.path.join(OUTPUT_PATH, "clases_transables_by_cluster_naics_2017.json"),"w"))