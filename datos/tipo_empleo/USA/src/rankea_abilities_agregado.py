...
CIIU_DATA_PATH = os.path.abspath(os.path.join(PATH_FILE, "..", "datos"))
...
##-------------------------------------------##
##-------------------------------------------##
## AGREGACIÓN A ABILITIES
##-------------------------------------------##
##-------------------------------------------##

abilities["Element ID"] = abilities[ "Element ID"].apply(lambda x : x[:3])
skills = abilities.copy()

skills = skills.groupby(["O*NET-SOC Code", "Element ID", "Scale ID"]).mean().reset_index()
...
## Isic rev 4
isic = pd.read_csv( os.path.join(CIIU_DATA_PATH,"ISICRev4_NT_input.csv"))
